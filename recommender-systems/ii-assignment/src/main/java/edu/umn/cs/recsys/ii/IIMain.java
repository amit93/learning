...
public class IIMain
{
	...
	public static void main(String[] args)
	{
		...
		if (basket != null)
		{
			...
			{
				System.out.format(Locale.ROOT, "%d,%.4f,%s\n", item.getId(), item.getScore(), titleDAO.getItemTitle(item.getId()));
			}
			return;
		}
	...
	public static Map<Long, String> basket(String[] args)
	{
		Set<Long> basket = null;
		if (args.length > 0)
		{
		...
		Map<Long, String> similarItems = Maps.newHashMap();
		...
		if (basket != null)
		{
			...
			for (ScoredId item: items)
			{
				...
				similarItems.put(item.getId(), score);
			}
		}
		return similarItems;
	}
