...
public class SVDItemScorer extends AbstractItemScorer
{
	...
	/**
	 * Score items in a vector. The key domain of the provided vector is the items to score, and the score method sets
	 * the values for each item to its score (or unsets it, if no score can be provided). The previous values are
	 * discarded.
	 * 
	...
	public void score(long user, @Nonnull MutableSparseVector scores)
	{
		...
		for (VectorEntry e: scores.fast(VectorEntry.State.EITHER))
		{
			...
			{
				scores.unset(e);
				logger.debug("No user feature vector for {}. Unsetting the score.", user);
			}
			else
			{
				...
				logger.trace("User feature vector: {},{}", userVector.getRowDimension(), userVector.getColumnDimension());
				logger.trace("Item feature vector: {},{}", itemVector.getRowDimension(), itemVector.getColumnDimension());
				logger.trace("Weights  vector: {},{}", weights.getRowDimension(), weights.getColumnDimension());

				RealMatrix temp = userVector.multiply(weights);

				// The result will be a 1 X 1 matrix
				RealMatrix result = temp.multiply(itemVector.transpose());

				double baselineScore = baselineScorer.score(user, item);

				// TODO Set the scores
				scores.set(e, baselineScore + result.getEntry(0, 0));
			}
		}
	}
