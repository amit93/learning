...
public class SVDMain
{
	...
	private LenskitConfiguration configureRecommender()
	{
		...
		config.bind(ItemScorer.class).to(SVDItemScorer.class);
		baselineMode.configure(config);
		config.set(LatentFeatureCount.class).to(10);
		return config;
	}
	...
	public Map<Long, String> run()
	{
		...
		Map<Long, String> predictions = Maps.newHashMap();
